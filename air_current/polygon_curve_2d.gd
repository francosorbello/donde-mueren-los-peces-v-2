...
enum CreationStrategy {
	EXTRUDE_FROM_PATH, # the polygon is generated by adding points along the defined path based on width
	POLYGON_FROM_PATH, # the polygon is generated based on the path itself. The path must be closed for it to work
}
...
@export var creation_strategy : CreationStrategy = CreationStrategy.EXTRUDE_FROM_PATH
...
func create():
	if creation_strategy == CreationStrategy.POLYGON_FROM_PATH:
		_create_from_path()
		return
...
func _create_from_path():
	...
	var points := curve.get_baked_points()
	assert(points[0] == points[points.size()-1], "Path on %s is not closed"%self.name)
